...
test("execute end-to-end test flow @end-to-end", async({page})=>{
    ...
    //Step1: Register a new account and capture the generated email
    let registeredEmail:string = await performRegistration(page);
    console.log("✅ Registration is completed!");

    //Step2: Logout after successful Registration
    await performLogout(page);
    console.log("✅ Logout is completed!");

    //Step3: Login with the registered email
    await performLogin(page, registeredEmail);
    console.log("✅ Login is completed!");

    //Step4: Search for a product and add it to the cart
    await addProductToCart(page);
    console.log("✅ Product added to cart!");

    //Step5: Verify the contents of the shopping cart
    await verifyShoppingCart(page);
    console.log("✅ Shopping cart verification completed!");

    //Step6: Perform checkout(skipped for demo site)
...
async function performRegistration(page:Page):Promise<string>
{
    ...
    await registrationPage.setPrivacyPolicy();//Accept Privacy setPrivacyPolicy
    await registrationPage.clickContinuebtn();//Submit the registration form

    //Validate the registration was successful
...
async function verifyShoppingCart(page:Page)
{
    ...
    //Validate the total price is correct(based on config)
...
{
    //Checkout feature is not implemented since it is a demo site
    //Place your checkout flow login here if backend is available
}